...
import type { CreateEventDto, EventListQuery, UpdateEventDto } from "./events.schemas.js";
...
export class EventsService {
  ...
  async list(query: EventListQuery) {
    ...
    const [events, total] = await this.prisma.$transaction([
      this.prisma.event.findMany({
        ...
        include: { _count: { select: { registrations: { where: { status: RegistrationStatuses.active } } } } }
      }),
  ...
  async update(id: string, dto: UpdateEventDto) {
    ...
    if (dto.pointsForParticipation !== undefined) data.pointsForParticipation = dto.pointsForParticipation;
  ...
  async registerUser(eventId: string, userId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (event.status !== EventStatuses.published) throw badRequest("Registration is available only for published events");
      if (event._count.registrations >= event.participantLimit) throw conflict("Participant limit reached");

      const existing = await tx.eventRegistration.findUnique({ where: { userId_eventId: { userId, eventId } } });
      if (existing?.status === RegistrationStatuses.active) throw conflict("User is already registered for this event");
  ...
  async cancelRegistration(eventId: string, userId: string) {
    const registration = await this.prisma.eventRegistration.findUnique({ where: { userId_eventId: { userId, eventId } } });
    if (!registration || registration.status !== RegistrationStatuses.active) throw notFound("Active registration not found");
